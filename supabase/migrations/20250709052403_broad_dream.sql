/*
  # Simplified Grading System Schema

  1. Enhanced Tables
    - Updated `question_papers` with question paper type support
    - Enhanced `student_submissions` with new fields
    - Improved `grading_sessions` with batch processing
    - Updated `ocr_jobs` for better tracking
...
-- Enhanced question_papers table
DO $$
BEGIN
  -- Add new columns if they don't exist
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'question_papers' AND column_name = 'has_separate_question_paper') THEN
    ALTER TABLE question_papers ADD COLUMN has_separate_question_paper boolean DEFAULT true;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'question_papers' AND column_name = 'question_paper_file_id') THEN
    ALTER TABLE question_papers ADD COLUMN question_paper_file_id uuid REFERENCES documents(id) ON DELETE SET NULL;
  END IF;
END $$;
